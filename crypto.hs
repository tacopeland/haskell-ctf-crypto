...
mulmod (x:xs) m = Just (ofIntegral (x * p) m)
    where Just (ZmodN p m) = mulmod xs m
...
--bsgs :: ZmodN -> ZmodN -> ZmodN -> Maybe Int
bsgs g h ord =
    let 
    ...
    in
        case intersect bstep gstep of
            [] -> Nothing
            (match:_) -> flip mod (fromIntegral $ bareInteger ord) <$> val
